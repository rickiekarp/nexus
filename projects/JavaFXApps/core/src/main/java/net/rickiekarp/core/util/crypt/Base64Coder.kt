...
object Base64Coder {
     ...
     * @return     A character array with the Base64 encoded data.
     */
    private fun encode(`in`: ByteArray, iLen: Int): CharArray {
        ...
        var op = 0
        while (ip < iLen) {
            val i0 = `in`[ip++] and 0xff
            val i1 = if (ip < iLen) `in`[ip++] and 0xff else 0
            val i2 = if (ip < iLen) `in`[ip++] and 0xff else 0
            val o0 = i0.ushr(2)
     ...
     * @throws    IllegalArgumentException if the input is not valid Base64 encoded data.
     */
    private fun decode(`in`: CharArray): ByteArray {
        var iLen = `in`.size
