...
/**
 * Utility class which creates a [LoginServerApplication] instance and runs the server from the command line
 */
class LoginServer {
    private val BASE_URI = UriBuilder.fromUri("http://0.0.0.0").port(8080).path("/" + LoginServer::class.java.simpleName).build()
    ...
    private fun startServer() {
        ...
        registration.setInitParameter("javax.ws.rs.Application", LoginServerApplication::class.java.name)
        registration.addMapping("/" + LoginServer::class.java.simpleName + "/api/*")
